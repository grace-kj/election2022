...
def train(args,
    ...
    logger.info("  learning_rate = %.6f", args.learning_rate)
...
def evaluate(args, model, eval_dataset, mode, global_step=None):
    ...
    with open(os.path.join('/content/drive/MyDrive/Colab Notebooks/선거인단 koelectra/koelectra results',str(args.test_file)[:7]+ '.tsv'),'w',newline='') as f:
      write= csv.writer(f)
      write.writerows(ret_list)
    #
...
############################################################
#evaluate 수정
def evaluate_dev(args, model, eval_dataset, mode, global_step=None):
    results = {}
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size)
    ...
    #
    ret_list.append(preds)
    with open('/content/drive/MyDrive/Colab Notebooks/선거인단 koelectra/results.tsv','w',newline='') as f:
      write= csv.writer(f)
      write.writerows(ret_list)
    #
...
def main(cli_args):
    ...
    if cli_args.bs:
        args.train_batch_size = cli_args.bs
    
    if cli_args.lr:
        args.learning_rate = cli_args.lr

    if cli_args.train:
        args.do_train = cli_args.train

    if cli_args.data:
        args.test_file =  cli_args.data

    args.gradient_accumulation_steps = cli_args.ac
    ...
    if args.do_eval:
        ...
        with open(output_eval_file, "w") as f_w:
            for key in sorted(results.keys()):
                f_w.write("{} = {}\n".format(key, str(results[key])))
    #
    #evaluate_dev(args, model, test_dataset, "dev", global_step)
...
if __name__ == '__main__':
    ...
    cli_parser.add_argument("--bs", type=int, default=None)
    cli_parser.add_argument("--lr", type=float, default=None)
    cli_parser.add_argument("--ac", type=int, default=1)
    cli_parser.add_argument("--config_file", type=str, required=True)
    cli_parser.add_argument("--train", type=lambda x:bool(strtobool(x)), default= True)
    cli_parser.add_argument("--data", type= str, default= None)

    cli_args = cli_parser.parse_args()


    fileHandler = logging.FileHandler('./' + cli_args.task + '_result.log')
